...
do
	name=($i)
	awk '{print $2}' $name"_ARGs_80.mapstat" | grep -v '#' | grep -v '[a-z]' > $name"_counts"
	awk '{print $1}' $name"_ARGs_80.mapstat" | grep -v '#' > $name"_genenames"
	...
	paste uniq_taxa_allsamples_header  *_temp > mapstat_80_mat.txt
	rm -f *_temp
