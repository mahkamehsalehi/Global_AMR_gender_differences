...
#SBATCH --array=4-100   # Total number of arrays
    ...
    if grep -Fxq "$accession" "$completed_list"; then
	    ...
	    cd ..
            ...
            if [[ -s "${accession}_1.fastq.gz" ]]; then
                ...
                if [ $? -eq 0 ]; then
                    echo "Removed ${accession}_2.fastq.gz successfully."
                else
                    echo "Failed to remove ${accession}_2.fastq.gz."
                fi
            elif [[ -s "${accession}.fastq.gz" ]]; then
            ...
            elif [[ -s "${accession}_1.fastq" ]]; then
                pigz -p 4 "${accession}_1.fastq"
                rm -f "${accession}_1.fastq"
                if [ $? -eq 0 ]; then
                    echo "Removed ${accession}_1.fastq successfully."
                else
                    echo "Failed to remove ${accession}_1.fastq."
                fi
                rm -f "${accession}_2.fastq.gz"
                if [ $? -eq 0 ]; then
                    echo "Removed ${accession}_2.fastq.gz successfully."
                ...
                echo "No valid FASTQ files found for accession $accession."
