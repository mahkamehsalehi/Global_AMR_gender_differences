...
hic_effect_sizes <- lapply(unique(metadata_hic$age_category_new), function(age) {
  ...
  if(nrow(hic_data) >= 2) {
    test <- wilcox.test(ARG_load ~ gender, data = hic_data)
    res <- wilcox_effsize(data = hic_data, ARG_load ~ gender)
...
lmic_effect_sizes <- lapply(unique(metadata_lmic$age_category_new), function(age) {
  ...
  if(nrow(lmic_data) >= 2) {
    test <- wilcox.test(ARG_load ~ gender, data = lmic_data)
    res <- wilcox_effsize(data = lmic_data, ARG_load ~ gender)
...
# Create the plots with tag levels
combined_plot <- (age_arg_boxplot_hic + labs(title = "HIC")) +
  (age_arg_boxplot_lmic + labs(title = "LMIC")) +
  plot_layout(ncol = 1, heights = c(1, 1), guides = "collect") +
  plot_annotation(
...
# Save the final plot
ggsave("RESULTS/FIGURES/ARG_load_income.jpg", 
       final_plot, 
       width = 12, 
       height = 25,
       dpi = 300)
