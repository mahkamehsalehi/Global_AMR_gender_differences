...
results_load <- df %>%
  ...
  summarise(
    N1 = sum(gender == "Women"),
    N2 = sum(gender == "Men"),
    Group1_Median = median(log10_ARG_load[gender == "Women"], na.rm = TRUE),
    Group2_Median = median(log10_ARG_load[gender == "Men"], na.rm = TRUE),
    P_value = wilcox.test(
      log10_ARG_load[gender == "Women"],
      log10_ARG_load[gender == "Men"],
      exact = FALSE
    )$p.value,
    .groups = "drop"
  ) %>%
