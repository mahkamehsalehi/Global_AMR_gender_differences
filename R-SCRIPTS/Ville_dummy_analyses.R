...
for(ic in 0:1) {
  ...
  if(ic == 0) {
    ...
    my_formula <- paste0("log_ARG_load ~ ",
                         paste0(dummy_var_names[!grepl(pattern = "Oceania|North_America|Africa|Asia|Infant|HIC",
                                                       x = dummy_var_names)],
                                collapse = "+")) %>% as.formula()
  }
  ...
  if(ic == 1) {
    # Remove South America
    my_formula <- paste0("log_ARG_load ~ ",
                         paste0(dummy_var_names[!grepl(pattern = "South_America|Africa|Asia|Infant|HIC",
                                                       x = dummy_var_names)],
                                collapse = "+")) %>% as.formula()
  }
...
saveRDS(object = fit_list_log_ARG_load, file = "fit_list_log_ARG_load2.RDS")
# fit_list_log_ARG_load <- readRDS(file = "fit_list_log_ARG_load2.RDS")
...
saveRDS(object = fit_list_shannon, file = "fit_list_shannon2.RDS")
# fit_list_shannon <- readRDS(file = "fit_list_shannon2.RDS")
...
full_summary$Feature <- gsub("age_category", "", full_summary$Feature)
full_summary$Feature <- gsub("Usage_high", "Antibiotic Use Low/High", full_summary$Feature)
full_summary$Feature <- gsub("sex_num_Men", "Man/Woman", full_summary$Feature)
...
## Full table
write.csv(full_summary_round, "ARG_shannon_dummy_lm2.csv")

## Log ARG load
full_summary_round %>% 
  filter(Response == "log(ARG load)") %>%
  select(-Response) %>% 
  write.csv(., "ARG_dummy_lm2.csv")

## Shannon
full_summary_round %>% 
  filter(Response == "Shannon") %>% 
  select(-Response) %>% 
  write.csv(., "shannon_dummy_lm2.csv")
