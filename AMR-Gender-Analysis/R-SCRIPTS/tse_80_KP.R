...
# working_directory <- "/scratch/project_2008149/USER_WORKSPACES/mahkameh/work"
# setwd(working_directory)

# Load data
gene_expression_data_80 <- fread("ALL_mapstat_80_mat.txt", header = TRUE, sep = "\t")
# gene_expression_data_90 <- fread("ALL_mapstat_90_mat.txt", header = TRUE, sep = "\t")
resfinder_phenotypes <- fread("resfinder_phenotypes-pjji7t1qo3nedped5943f1c1wy.txt", header = TRUE, sep = "\t", fill = TRUE)
gene_lengths <- fread("gene_length.csv", header = TRUE)
metadata <- fread("Sra_metadata_jun11.txt", header = TRUE, sep = ",")
...
# # Save the TreeSummarizedExperiment object
 saveRDS(tree_summarized_experiment, file="TSE.rds")
